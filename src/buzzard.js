function __buzzard(global, lib) {
    ...
    var str_mem = new global.Int8Array(STR_MEM_SIZE);
    var m = new global.Int32Array(MEM_SIZE);
    ...
    var last_dict_entry = 1,
        program_counter,
        stack = new global.Int32Array(STACK_SIZE),
        stack_ptr = 0,
     ...
     function append_to_dict(val) {
         var addr = m[0];
         m[addr] = val;
         m[0] += 1;
     }

     function def_word(codeword) {
         append_to_dict(last_dict_entry);
         last_dict_entry = m[0] - 1;
         append_to_dict(last_str_entry);
         append_to_dict(codeword);
         // readWordInto(array of chars, offset to start at) -> string length read
         var readLength = lib.readWordInto(str_mem, last_str_entry);
         // the addition by 1 is for the \0 since we are simulating C strings
         last_str_entry += readLength + 1;
     }

     function r(word_addr) {
         var read_count, val, entry_addr, entry_data_addr;
         var next_word = word_addr + 1;
         var codeword = m[word_addr];
         switch (codeword) {
             case CW__READ: // _read
                 // first 64 bytes of str_mem are used to read user input, if
                 // word is larger than that it will overwrite word names
                 read_count = lib.readWordInto(str_mem, 0);
                 ...
                 while (lib.strcmp(str_mem, 0, str_mem, m[entry_addr + 1])) {
             ...
             case CW_KEY: // key
                 stack_ptr += 1;
                 stack[stack_ptr] = top_of_stack;
                 top_of_stack = lib.getchar();
         }
     }

     function main() {
         var i, tmp1, word_to_execute;
     ...
     return {
         ...
         m: m,
         stack: stack,
         last_dict_entry: function () {
             return last_dict_entry;
         }
     };
}
